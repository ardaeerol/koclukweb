...
const Chat = () => {
  ...
  useEffect(() => {
    const fetchChats = async () => {
      try {
        const response = await api.get('/chats/coach'); // Backend'de '/chats/coach' endpoint
        ...
        <div className="chat-window">
          ...
          <div className="messages">
            {chats
              ...
              ?.messages.map((msg) => (
                <div key={msg.chatId} className={`message ${msg.senderId === /* current user id */ 2 ? 'sent' : 'received'}`}>
                  <p>{msg.message}</p>
